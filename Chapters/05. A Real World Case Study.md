...
Transformer Block with Multi-Head Latent Attention (MLA) and DeepSeekMoE
```mermaid
flowchart LR
...
%% Transformer Block
%% =========================
subgraph TBLOCK["Transformer Block × L"]
direction TB
...
MLA_OUT["Output Hidden uₜ"]

CACHE["KV Cache (Inference)"]
...
subgraph MOE["DeepSeekMoE"]

direction TB

MOEIN["Input Hidden uₜ"]
...
SUM["Merge Expert Outputs"]

MOEOUT["Output Hidden h′ₜ"]
...
classDef block fill:#FFF5D6,stroke:#444,stroke-width:1px,color:#000;
classDef latent fill:#E7F2FF,stroke:#2E6FD8,stroke-width:1px;
classDef expert fill:#EAF8E5,stroke:#5A9C42,stroke-width:1px;
classDef route fill:#FFE8C6,stroke:#C97A00,stroke-width:1px;
classDef cache fill:#F4F4F4,stroke:#777,stroke-dasharray:5 5;

class RMS1,RMS2,ATTN,FFN,MHA block;
class LATQ,LATKV,Q,K,V,ROPEQ,ROPEK,CONQ,CONKV latent;
class E1,E2,E3,E4,EN,SE expert;
class ROUTER,TOPK route;
class CACHE cache;
```